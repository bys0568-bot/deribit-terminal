...
def get_monthly_expiration(valid_expirations, now_utc):
  monthly_candidates = []
  for ts in valid_expirations:
    dt = datetime.fromtimestamp(ts / 1000, tz=timezone.utc)
    if dt.weekday() == 4:  # Friday
      if (dt + timedelta(days=7)).month != dt.month:
        if ts > now_utc.timestamp() * 1000:
          monthly_candidates.append(ts)

  if monthly_candidates:
    return monthly_candidates[0]

  future_exps = [ts for ts in valid_expirations if ts > now_utc.timestamp() * 1000]
  return future_exps[-1] if future_exps else valid_expirations[0]


def get_weekly_expiration(valid_expirations, monthly_ts, now_utc):
  # 주간물은 반드시 금요일(weekday == 4)이면서 월물이 아닌 가장 가까운 미래 날짜
  weekly_candidates = []
  for ts in valid_expirations:
    dt = datetime.fromtimestamp(ts / 1000, tz=timezone.utc)
    if (
        dt.weekday() == 4
        and ts != monthly_ts
        and ts > now_utc.timestamp() * 1000
    ):
      weekly_candidates.append(ts)
  ...
  future_exps = [
      ts
      for ts in valid_expirations
      if ts > now_utc.timestamp() * 1000 and ts != monthly_ts
  ]
  return future_exps[0] if future_exps else valid_expirations[0]


def fetch_single_ticker(ins_name):
  try:
    res = requests.get(
    ...
    if 'result' in res:
      return ins_name, res['result']
  except Exception:
    pass
  return ins_name, None


@st.cache_data(ttl=300)
def load_all_market_data(currency):
  ...
  monthly_ts = get_monthly_expiration(valid_expirations, now_utc)
  weekly_ts = get_weekly_expiration(valid_expirations, monthly_ts, now_utc)
  ...
  all_inst_names = [inst['instrument_name'] for inst in instruments]
  ticker_map = {}
  with ThreadPoolExecutor(max_workers=20) as executor:
    results = executor.map(fetch_single_ticker, all_inst_names)
    for name, t_info in results:
      if t_info:
        ticker_map[name] = t_info

  return instruments, spot_price, dvol_val, targets, ticker_map, now_utc
...
def run_dashboard():
  ...
  with st.spinner(f'🔄 Fetching {currency} options data (Cached)...'):
    (
        instruments,
        spot_price,
        dvol_val,
        ...
        ticker_map,
        now_utc,
    ) = load_all_market_data(currency)

  if instruments is None:
    st.warning('⚠️ No valid option data found.')
    return

  tab1, tab2, tab3 = st.tabs(
      ['⚡ 0DTE / Micro', '📅 Weekly', '🏛️ Monthly / Macro']
  )
  tabs_dict = {
      '0DTE / Micro': tab1,
      'Weekly': tab2,
  ...
  for label, target_ts in targets.items():
    with tabs_dict[label]:
      target_dt_utc = datetime.fromtimestamp(target_ts / 1000, tz=timezone.utc)
      ...
      for inst in target_instruments:
        ...
        t_info = ticker_map.get(ins_name, {})
      ...
      otm_puts = df[(df['option_type'] == 'put') & (df['strike'] < spot_price)]
      otm_calls = df[(df['option_type'] == 'call') & (df['strike'] > spot_price)]
      ...
      for s in unique_strikes:
        call_pain = (
            np.maximum(0, s - df[df['option_type'] == 'call']['strike'])
            * df[df['option_type'] == 'call']['open_interest']
        ).sum()
        put_pain = (
            np.maximum(0, df[df['option_type'] == 'put']['strike'] - s)
      ...
      grouped = df.groupby('strike_grouped').sum(numeric_only=True).reset_index()
      ...
      for ax in [ax1, ax2]:
        ...
        ax.axvline(
            upper_1sig, color='orange', linestyle='-.', linewidth=1.2, alpha=0.7
        )
        ax.axvline(
            lower_1sig, color='orange', linestyle='-.', linewidth=1.2, alpha=0.7
        )
