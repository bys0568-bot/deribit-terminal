...
def get_monthly_expiration(valid_expirations, now_utc):
  # 데리빗 표준 월물: 해당 월의 마지막 금요일 (7일 뒤가 다음 달로 넘어가는 금요일)
  monthly_candidates = []
...
def run_dashboard():
  ...
  with st.spinner(f'🔄 Fetching {currency} options data (Optimized)...'):
    try:
      ...
      # 속도 최적화: 모든 옵션 데이터를 한 번의 요청(Batch)으로 가져옴
      ...
      summary_map = {item['instrument_name']: item for item in summaries}
      ...
      # 주간물: 0DTE 다음 만기 중 월물이 아닌 첫 번째 만기
      ...
      for label, target_ts in targets.items():
        with tabs_dict[label]:
          ...
          for inst in target_instruments:
            ...
            summary = summary_map.get(ins_name, {})
