...
# 5분(300초) 동안 데이터를 캐싱하여 탭 전환 시 즉시 로딩되도록 최적화
...
def load_all_market_data(currency):
  ...
  weekly_ts = nearest_ts
  for ts in valid_expirations:
    if ts > nearest_ts and ts != monthly_ts:
      weekly_ts = ts
      break
  if weekly_ts == nearest_ts and len(valid_expirations) > 1:
    weekly_ts = valid_expirations[1]
