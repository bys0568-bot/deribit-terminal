...
def run_dashboard():
  ...
  with st.spinner(f'🔄 Fetching {currency} options data (High-Speed)...'):
    try:
      ...
      for label, target_ts in targets.items():
        with tabs_dict[label]:
          ...
          # 멀티스레딩으로 해당 만기의 모든 티커 데이터를 동시에 빠르게 수집
          inst_names = [inst['instrument_name'] for inst in target_instruments]
          ticker_map = {}
          with ThreadPoolExecutor(max_workers=15) as executor:
            results = executor.map(fetch_single_ticker, inst_names)
